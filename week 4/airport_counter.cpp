...
public:
	...
	AirportCounter() { Airport.fill(0); }

	// конструктор от диапазона элементов типа TAirport
	template <typename TIterator>
	AirportCounter(TIterator begin, TIterator end): AirportCounter() {
		for (auto it = begin; it != end; it++) 
			Airport[static_cast<size_t>(*it)]++;
	}

	// получить количество элементов, равных данному
	size_t Get(TAirport a) const { return Airport[static_cast<size_t>(a)]; }

	// добавить данный элемент
	void Insert(TAirport a) { Airport[static_cast<size_t>(a)]++; }

	// удалить одно вхождение данного элемента
	void EraseOne(TAirport a) { Airport[static_cast<size_t>(a)]--; }

	// удалить все вхождения данного элемента
	void EraseAll(TAirport a) { Airport[static_cast<size_t>(a)] = 0; }
	...
	Items GetItems() const {
		...
		for (size_t i = 0; i < Airport.size(); ++i)
			res[i] = { static_cast<TAirport>(i), Airport[i] };
		return res;
	}

private:
	array<size_t, static_cast<size_t>(TAirport::Last_)> Airport;
};
