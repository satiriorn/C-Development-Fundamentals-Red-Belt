...
public:
  ...
  void Book(int64_t time, const string& hotel_name, unsigned int client_id, int room_count)
  {
  ...
  uint64_t GetClients(const string& hotel_name) {return clients[hotel_name].size();}

  uint64_t GetRooms(const string& hotel_name) { return rooms[hotel_name];}
};
