...
public:
  ReadingManager(): users_to_pages(MAX_USER_COUNT_ + 1, 0), pages_to_users(MAX_PAGES_ + 1, 0), pages_rating(MAX_PAGES_ + 1, 0.0){}
...
private:
  const int MAX_USER_COUNT_ = 100000;
  const int MAX_PAGES_ = 1000;

  vector<int> users_to_pages, pages_to_users;
